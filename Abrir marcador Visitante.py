...
def process_data(url):
    ...
    if response.status_code == 200:
        ...
        tabela_abre_marcador_visitante = soup.find_all("tbody", class_="loader-container")[1]

        informacoes_abre_marcador = {}
        linhas_abre_marcador = tabela_abre_marcador_visitante.find_all("tr")
        ...
        print("Estatísticas de Abrir o Marcador para a equipe visitante:")
    ...
    if '⇒ e está a vencer ao intervalo' in informacoes_abre_marcador:
        vence_intervalo = informacoes_abre_marcador["⇒ e está a vencer ao intervalo"].split()[0]
        if vence_intervalo.isdigit():
            vence_intervalo = int(vence_intervalo)
            ...
            if vence_intervalo > 0:
                ...
                print(f"A equipe está a vencer ao int 0 jogos em: {numero_golos_int}")
        else:
            print("Não há dados disponíveis para '⇒ e está a vencer ao intervalo'")

    if '⇒ e vence no final' in informacoes_abre_marcador:
        vence_final = informacoes_abre_marcador["⇒ e vence no final"].split()[0]
        if vence_final.isdigit():
            vence_final = int(vence_final)
            ...
            if vence_final > 0:
                ...
                print(f" A equipe vence no final dos jogos 0 em {numero_golos_int_final}")
        else:
            print("Não há dados disponíveis para '⇒ e vence no final'")
